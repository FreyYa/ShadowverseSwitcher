function Main
{
    begin
    {
        ...
        $target = 'Release'
        $result = 'ShadowverseSwitcher'
        $bin = '..\ShadowverseSwitcher\bin\'
        ...
        $exeSource  = 'ShadowServant.exe'
 
        if (-not(Test-Path $bin))
        {
            throw 'Script detected as locate in invalid path exception!! Make sure exist in <ShadowverseSwitcher repository root>\tools-release\'
        }
    }
